      ...
      <div className="flex-1 overflow-y-auto px-7 py-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            {form.photoUrl && (
              <button onClick={() => setForm(f => ({ ...f, photoUrl: '' }))}
                className="text-xs text-error hover:underline block">Retirer la photo</button>
            )}
        ...
        <div className="space-y-4">
          <p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest">
            Identité <span className="text-error font-normal normal-case">— champs obligatoires</span>
          </p>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={labelCls}>Drapeau <span className="font-normal normal-case opacity-60">(emoji, optionnel)</span></label>
              <input type="text" value={form.flag}
                onChange={e => setForm(f => ({ ...f, flag: e.target.value }))}
                className={inputCls()} placeholder="Ex : 🇫🇷" />
            </div>
          </div>
        ...
        <div className="space-y-4 pt-2 border-t border-outline-variant">
          <p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest">
            Informations personnelles <span className="font-normal normal-case opacity-60">(optionnel)</span>
          </p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={form.injury}
                  ...
                  className={inputCls()} placeholder={form.status === 'Blessé' ? 'Ex : Ischio-jambiers' : 'Ex : 2 matchs de suspension'} />
              </div>
              <div>
            ...
            <div className={`${PANEL_W} h-full bg-surface-container-lowest border border-outline-variant rounded-2xl flex flex-col overflow-hidden`}>
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-outline-variant shrink-0">
                ...
                <button onClick={() => openEdit(displayed)}
                  className="flex items-center gap-2 px-5 py-2.5 bg-error hover:bg-error/90 text-white text-base font-semibold rounded-xl transition-colors">
