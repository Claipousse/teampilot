...
const players = [
  {
    ...
    number: '08',
    formScore: 94,
    formColor: 'text-secondary',
    status: 'FIT',
    statusColor: 'bg-secondary',
    tags: ['CM', 'REGISTA'],
    trend: '+2.4%',
    trendColor: 'text-secondary',
    insight: 'AI: Peak performance projected for Matchday',
    insightIcon: '✦',
    initials: 'MV',
    sleep: 8.2,
    fuel: 94,
    recovery: 76,
    projectedIntensity: 'Elite (Level 5)',
    aerobicCapacity: 'Optimal',
    matchReadiness: 98,
  ...
  },
  {
    ...
    number: '03',
    formScore: 62,
    formColor: 'text-error',
    status: 'FATIGUE',
    statusColor: 'bg-tertiary-container',
    tags: ['LB', 'LWB'],
    trend: '-1.2%',
    trendColor: 'text-error',
    insight: 'AI: High injury risk (Hamstring)',
    insightIcon: '⚠',
    alert: true,
    initials: 'JR',
    sleep: 6.1,
    fuel: 72,
    recovery: 45,
    projectedIntensity: 'Low (Level 2)',
    aerobicCapacity: 'At Risk',
    matchReadiness: 40,
  ...
  },
  {
    ...
    number: '09',
    formScore: 81,
    formColor: 'text-on-surface',
    status: 'FIT',
    statusColor: 'bg-secondary',
    tags: ['ST', 'CF'],
    trend: 'STABLE',
    trendColor: 'text-on-surface-variant',
    insight: 'Tactical Match: High pressing efficiency',
    insightIcon: '✓',
    initials: 'KL',
    sleep: 7.8,
    fuel: 88,
    recovery: 82,
    projectedIntensity: 'High (Level 4)',
    aerobicCapacity: 'Good',
    matchReadiness: 85,
  ...
  },
  {
    ...
    trend: '+0.8%',
    trendColor: 'text-secondary',
    insight: 'Alert: High distribution accuracy this week',
    insightIcon: '⚡',
    initials: 'SK',
    sleep: 8.5,
    fuel: 91,
...
export default function JoueursPage() {
  ...
  return (
    <div className="flex gap-5 h-full">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 mb-5 p-4 bg-surface-container-lowest border border-outline-variant rounded-2xl">
          <div className="flex flex-col">
            <span className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-1">Position</span>
            <button className="flex items-center gap-1 text-base font-semibold text-on-surface">
              All Positions <ChevronDown size={16} />
            </button>
          </div>
          <div className="w-px h-8 bg-outline-variant" />
          <div className="flex flex-col">
            <span className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-1">Availability</span>
            <button className="flex items-center gap-1 text-base font-semibold text-on-surface">
              Full Squad <ChevronDown size={16} />
            </button>
          </div>
          <div className="w-px h-8 bg-outline-variant" />
          <div className="flex flex-col">
            <span className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-1">Sort By</span>
            <button className="flex items-center gap-1 text-base font-semibold text-on-surface">
              Form Score <ChevronDown size={16} />
            </button>
          </div>
          <div className="w-px h-8 bg-outline-variant" />
          ...
          {filtered.map(player => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                <span className={`px-3 py-1 rounded-full text-sm font-bold text-white ${player.statusColor}`}>
                  {player.status}
                </span>
                {player.tags.map(tag => (
                  <span key={tag} className="px-3 py-1 rounded-full text-sm font-bold bg-surface-container text-on-surface-variant">
                    {tag}
                  </span>
                ))}
                <span className={`text-sm font-bold ml-auto ${player.trendColor}`}>
                  {player.trend}
                </span>
              </div>
      ...
      >
        {displayedPlayer && (
          <div className="w-[600px] h-full bg-surface-container-lowest border border-outline-variant rounded-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="flex items-center gap-5">
                <div className="w-20 h-20 rounded-xl bg-surface-container-high flex items-center justify-center shrink-0">
                  <span className="text-3xl font-bold text-on-surface-variant">{displayedPlayer.initials}</span>
                </div>
                <div>
                  <p className="text-3xl font-bold text-on-surface">{displayedPlayer.name}</p>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-surface-container rounded-xl p-4">
                    <p className="text-sm text-on-surface-variant mb-1">🌙 Sleep</p>
                    <p className="text-2xl font-bold text-on-surface">{displayedPlayer.sleep}<span className="text-base font-normal">h</span></p>
                    <div className="h-2 bg-surface-container-high rounded-full mt-2">
                      <div className="h-2 bg-primary rounded-full" style={{ width: `${(displayedPlayer.sleep / 10) * 100}%` }} />
                    </div>
                  </div>
                  <div className="bg-surface-container rounded-xl p-4">
                    <p className="text-sm text-on-surface-variant mb-1">🍽 Fuel</p>
                    <p className="text-2xl font-bold text-on-surface">{displayedPlayer.fuel}<span className="text-base font-normal">%</span></p>
                    <div className="h-2 bg-surface-container-high rounded-full mt-2">
                      <div className="h-2 bg-secondary rounded-full" style={{ width: `${displayedPlayer.fuel}%` }} />
                    </div>
                  </div>
                  <div className="bg-surface-container rounded-xl p-4">
                    <p className="text-sm text-on-surface-variant mb-1">🔄 Recovery</p>
                    <p className={`text-2xl font-bold ${displayedPlayer.recovery < 60 ? 'text-error' : 'text-on-surface'}`}>
                      {displayedPlayer.recovery}<span className="text-base font-normal">%</span>
                    </p>
                    <div className="h-2 bg-surface-container-high rounded-full mt-2">
                      <div
                        className={`h-2 rounded-full ${displayedPlayer.recovery < 60 ? 'bg-error' : 'bg-secondary'}`}
                        style={{ width: `${displayedPlayer.recovery}%` }}
                      />
                    </div>
                  </div>
                </div>
              ...
              <div>
                <p className="text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-3">
                  Performance Prediction (Next 7 Days)
                </p>
                <div className="bg-inverse-surface rounded-xl p-5">
                  <p className="text-sm text-white/40 uppercase tracking-widest mb-1">Projected Intensity</p>
                  <div className="flex items-center justify-between mb-5">
                    <p className="text-2xl font-bold text-white">{displayedPlayer.projectedIntensity}</p>
              ...
              {/* AI Insight */}
              <div
                className="rounded-xl overflow-hidden border border-[#F97316]/30 border-l-4 border-l-[#F97316]"
              ...
              >
                <div className="p-5">
                  ...
                  <p className="text-xl font-bold text-on-surface mb-2">Tactical Optimization</p>
                  ...
                  <div className="flex gap-3">
                    <button className="flex-1 py-3 border border-outline-variant rounded-xl text-base font-semibold text-on-surface hover:bg-surface-container transition-colors bg-white">
                      Dismiss
                    </button>
                    <button className="flex-1 py-3 bg-primary hover:bg-primary-container text-white text-base font-semibold rounded-xl transition-colors">
                      Apply to Schedule
                    </button>
                  </div>
                </div>
